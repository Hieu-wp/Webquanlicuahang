...
const PRODUCT_API   = "/api/staff-products";
...
let currentCustomerId    = null; // ⭐ ID khách hàng đã chọn qua autocomplete
...
async function luuHoaDon() {
  const customerId = Number(currentCustomerId || 0); // ⭐ lấy từ autocomplete
  ...
  const payload = {
    ...
    idnhanvien: currentUser?.idnhanvien || null,
    ngaygiaodich: ngay,
    tongthanhtoan: tong,
...
async function xemChiTietHoaDonById(id) {
  try {
    ...
    let tenNV = data.tennhanvien || "";
    if (!tenNV && data.idnhanvien && staffMap[data.idnhanvien]) {
      tenNV = staffMap[data.idnhanvien];
    }
    if (
      !tenNV &&
      currentUser &&
      (currentUser.vaitro === "employee" || currentUser.vaitro === "staff")
    ) {
      tenNV = currentUser.hoten || currentUser.tendangnhap || "";
    }
...
async function loadCustomers() {
  try {
    ...
    if (!res.ok) return;
    const data = await res.json();
    allCustomers = data || []; // ⭐ chỉ lưu, không tạo <option> nữa
  } catch (e) {
    console.error(e);
  }
}
...
function setupCustomerAutocomplete() {
  ...
  input.addEventListener("input", () => {
    ...
    currentCustomerId = null; // gõ lại coi như chưa chọn
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (user.vaitro !== "employee" && user.vaitro !== "staff") {
  ...
  // setup autocomplete trước, data sẽ đổ vào allCustomers sau
